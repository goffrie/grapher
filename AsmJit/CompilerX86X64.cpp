...
void EInstruction::_emit(Assembler& a) ASMJIT_NOTHROW
{
  a._comment = _comment;
...
void EJmp::_emit(Assembler& a) ASMJIT_NOTHROW
{
...
void EFunction::_emitProlog(CompilerContext& cc) ASMJIT_NOTHROW
{
  ...
  if (!_isNaked)
  {
    _compiler->_emit(INST_PUSH, nbp);
    _compiler->_emit(INST_MOV, nbp, nsp);
  }
  ...
  if (_isStackAlignedByFnTo16Bytes)
  {
    ...
    _compiler->_emit(INST_AND, nsp, imm(-16));
  }
  ...
  if (preservedGP && _pePushPop)
  {
    ...
    {
      if (preservedGP & mask) _compiler->_emit(INST_PUSH, gpn(i));
    }
  }
  ...
  if (_isEspAdjusted)
  {
    ...
    if (stackSubtract) _compiler->_emit(INST_SUB, nsp, imm(stackSubtract));
  }
  else
  {
  ...
  if (preservedXMM)
  {
    for (i = 0, mask = 1; i < REG_NUM_XMM; i++, mask <<= 1)
    {
      ...
      {
        _compiler->_emit(_movDqaInstruction, dqword_ptr(nsp, nspPos), xmm(i));
        nspPos += 16;
      }
    }
  }
  ...
  if (preservedMM)
  {
    for (i = 0, mask = 1; i < 8; i++, mask <<= 1)
    {
      if (preservedMM & mask)
      {
        _compiler->_emit(INST_MOVQ, qword_ptr(nsp, nspPos), mm(i));
        nspPos += 8;
      }
    }
  }
  ...
  if (preservedGP && !_pePushPop)
  {
    for (i = 0, mask = 1; i < REG_NUM_GP; i++, mask <<= 1)
    {
      if (preservedGP & mask)
      {
        _compiler->_emit(INST_MOV, sysint_ptr(nsp, nspPos), gpn(i));
...
void EFunction::_emitEpilog(CompilerContext& cc) ASMJIT_NOTHROW
{
  ...
  if (preservedXMM)
  {
    for (i = 0, mask = 1; i < REG_NUM_XMM; i++, mask <<= 1)
    {
      if (preservedXMM & mask)
      {
        _compiler->_emit(_movDqaInstruction, xmm(i), dqword_ptr(nsp, nspPos));
        nspPos += 16;
      }
    }
  }
  ...
  if (preservedMM)
  {
    for (i = 0, mask = 1; i < 8; i++, mask <<= 1)
    {
      if (preservedMM & mask)
      {
        _compiler->_emit(INST_MOVQ, mm(i), qword_ptr(nsp, nspPos));
        nspPos += 8;
      }
    }
  }
  ...
  if (preservedGP && !_pePushPop)
  {
    for (i = 0, mask = 1; i < REG_NUM_GP; i++, mask <<= 1)
    {
      if (preservedGP & mask)
      {
        _compiler->_emit(INST_MOV, gpn(i), sysint_ptr(nsp, nspPos));
        nspPos += sizeof(sysint_t);
      }
    }
  }
  ...
  if (_isEspAdjusted && stackAdd != 0)
    _compiler->_emit(INST_ADD, nsp, imm(stackAdd));
  ...
  if (preservedGP && _pePushPop)
  {
    for (i = REG_NUM_GP - 1, mask = 1 << i; (int32_t)i >= 0; i--, mask >>= 1)
    {
      if (preservedGP & mask)
      {
        _compiler->_emit(INST_POP, gpn(i));
      }
    }
  }

  // Emit Emms.
  if (_emitEMMS) _compiler->_emit(INST_EMMS);

  // Emit SFence / LFence / MFence.
  if ( _emitSFence &&  _emitLFence) _compiler->_emit(INST_MFENCE); // MFence == SFence & LFence.
  if ( _emitSFence && !_emitLFence) _compiler->_emit(INST_SFENCE); // Only SFence.
  if (!_emitSFence &&  _emitLFence) _compiler->_emit(INST_LFENCE); // Only LFence.
  ...
  if (!_isNaked)
  {
    if (cpuInfo->vendorId == CPU_VENDOR_AMD)
    {
      ...
      _compiler->_emit(INST_LEAVE);
    }
    else
    {
      _compiler->_emit(INST_MOV, nsp, nbp);
      _compiler->_emit(INST_POP, nbp);
    }
  }

  // Emit return using correct instruction.
  if (_functionPrototype.getCalleePopsStack())
    _compiler->_emit(INST_RET, imm((int16_t)_functionPrototype.getArgumentsStackSize()));
  else
    _compiler->_emit(INST_RET);
}
          ...
          if ((getVariableClass(vdst->type) & VariableInfo::CLASS_GP) != 0)
          {
            ...
            if (x == INVALID_VALUE && (rdst->flags & VarCallRecord::FLAG_CALL_OPERAND_REG) != 0)
            {
              ...
              if (rIndex < REG_NUM_GP)
              {
                ...
                compiler->_emit(INST_MOV, gpn(rIndex), gpn(vsrc->registerIndex));
              ...
              if (getVariableClass(dstArgType.variableType) == getVariableClass(srcArgType.variableType))
              {
                ...
                if (srcIndex == dstArgType.registerIndex)
                {
                  ...
                  if (vdst->type != VARIABLE_TYPE_GPD || vsrc->type != VARIABLE_TYPE_GPD)
                    compiler->_emit(INST_XCHG, gpq(dstIndex), gpq(srcIndex));
                  else
#endif
                    compiler->_emit(INST_XCHG, gpd(dstIndex), gpd(srcIndex));
...
void ECall::_moveAllocatedVariableToStack(CompilerContext& cc, VarData* vdata, const FunctionPrototype::Argument& argType) ASMJIT_NOTHROW
{
  ...
  switch (vdata->type)
  {
    case VARIABLE_TYPE_GPD:
      ...
      {
        case VARIABLE_TYPE_GPD:
          compiler->_emit(INST_MOV, dst, gpd(src));
          return;
#if defined(ASMJIT_X64)
        case VARIABLE_TYPE_GPQ:
        case VARIABLE_TYPE_MM:
          compiler->_emit(INST_MOV, dst, gpq(src));
          return;
#endif // ASMJIT_X64
      }
      break;
    ...
    case VARIABLE_TYPE_GPQ:
      switch (argType.variableType)
      {
        ...
        case VARIABLE_TYPE_GPQ:
          ...
          return;
        case VARIABLE_TYPE_MM:
          compiler->_emit(INST_MOVQ, dst, gpq(src));
          return;
      }
      break;
#endif // ASMJIT_X64
    ...
    case VARIABLE_TYPE_MM:
      switch (argType.variableType)
      {
        ...
        case VARIABLE_TYPE_X87_1F:
        case VARIABLE_TYPE_XMM_1F:
          compiler->_emit(INST_MOVD, dst, mm(src));
          return;
        case VARIABLE_TYPE_GPQ:
        case VARIABLE_TYPE_MM:
        case VARIABLE_TYPE_X87_1D:
        case VARIABLE_TYPE_XMM_1D:
          compiler->_emit(INST_MOVQ, dst, mm(src));
          return;
      }
      break;
    ...
    case VARIABLE_TYPE_XMM_2D:
      ...
      {
        case VARIABLE_TYPE_XMM:
          compiler->_emit(INST_MOVDQU, dst, xmm(src));
          return;
        case VARIABLE_TYPE_XMM_1F:
        case VARIABLE_TYPE_XMM_4F:
          compiler->_emit(INST_MOVUPS, dst, xmm(src));
          return;
        case VARIABLE_TYPE_XMM_1D:
        case VARIABLE_TYPE_XMM_2D:
          compiler->_emit(INST_MOVUPD, dst, xmm(src));
          return;
      }
      break;
    ...
    case VARIABLE_TYPE_XMM_1F:
      switch (argType.variableType)
      {
        ...
        case VARIABLE_TYPE_XMM_1D:
        case VARIABLE_TYPE_XMM_2D:
          compiler->_emit(INST_MOVSS, dst, xmm(src));
          return;
      }
      break;
    ...
    case VARIABLE_TYPE_XMM_1D:
      switch (argType.variableType)
      {
        ...
        case VARIABLE_TYPE_XMM_1D:
        case VARIABLE_TYPE_XMM_2D:
          compiler->_emit(INST_MOVSD, dst, xmm(src));
          return;
      }
      break;
  }
  ...
  uint32_t temporaryXmmReg) ASMJIT_NOTHROW
{
  ...
  switch (vdata->type)
  {
    case VARIABLE_TYPE_GPD:
      ...
      {
        case VARIABLE_TYPE_GPD:
          compiler->_emit(INST_MOV, gpd(temporaryGpReg), src);
          compiler->_emit(INST_MOV, dst, gpd(temporaryGpReg));
          return;
#if defined(ASMJIT_X64)
        case VARIABLE_TYPE_GPQ:
        case VARIABLE_TYPE_MM:
          compiler->_emit(INST_MOV, gpd(temporaryGpReg), src);
          compiler->_emit(INST_MOV, dst, gpq(temporaryGpReg));
          return;
#endif // ASMJIT_X64
      }
      break;
    ...
    case VARIABLE_TYPE_GPQ:
      switch (argType.variableType)
      {
        ...
        case VARIABLE_TYPE_GPQ:
        case VARIABLE_TYPE_MM:
          compiler->_emit(INST_MOV, gpq(temporaryGpReg), src);
          compiler->_emit(INST_MOV, dst, gpq(temporaryGpReg));
          return;
      }
      break;
    ...
    case VARIABLE_TYPE_XMM_2D:
      ...
      {
        case VARIABLE_TYPE_XMM:
          compiler->_emit(INST_MOVDQU, xmm(temporaryXmmReg), src);
          compiler->_emit(INST_MOVDQU, dst, xmm(temporaryXmmReg));
          return;
        case VARIABLE_TYPE_XMM_1F:
        case VARIABLE_TYPE_XMM_4F:
          compiler->_emit(INST_MOVUPS, xmm(temporaryXmmReg), src);
          compiler->_emit(INST_MOVUPS, dst, xmm(temporaryXmmReg));
          return;
        case VARIABLE_TYPE_XMM_1D:
        case VARIABLE_TYPE_XMM_2D:
          compiler->_emit(INST_MOVUPD, xmm(temporaryXmmReg), src);
          compiler->_emit(INST_MOVUPD, dst, xmm(temporaryXmmReg));
          return;
      }
      break;
    ...
    case VARIABLE_TYPE_XMM_1F:
      switch (argType.variableType)
      {
        ...
        case VARIABLE_TYPE_XMM_1D:
        case VARIABLE_TYPE_XMM_2D:
          compiler->_emit(INST_MOVSS, xmm(temporaryXmmReg), src);
          compiler->_emit(INST_MOVSS, dst, xmm(temporaryXmmReg));
          return;
      }
      break;
    ...
    case VARIABLE_TYPE_XMM_1D:
      switch (argType.variableType)
      {
        ...
        case VARIABLE_TYPE_XMM_1D:
        case VARIABLE_TYPE_XMM_2D:
          compiler->_emit(INST_MOVSD, xmm(temporaryXmmReg), src);
          compiler->_emit(INST_MOVSD, dst, xmm(temporaryXmmReg));
          return;
      }
      break;
  }
    ...
    switch (argType.variableType)
    {
      case VARIABLE_TYPE_GPD:
        switch (vdata->type)
        {
          ...
          case VARIABLE_TYPE_GPQ:
#endif // ASMJIT_X64
            compiler->_emit(INST_MOV, gpd(dst), gpd(src));
            return;
          case VARIABLE_TYPE_MM:
            compiler->_emit(INST_MOVD, gpd(dst), mm(src));
            return;
        }
        break;
      ...
      case VARIABLE_TYPE_GPQ:
        switch (vdata->type)
        {
          case VARIABLE_TYPE_GPD:
            ...
            return;
          case VARIABLE_TYPE_GPQ:
            compiler->_emit(INST_MOV, gpq(dst), gpq(src));
            return;
          case VARIABLE_TYPE_MM:
            compiler->_emit(INST_MOVQ, gpq(dst), mm(src));
            return;
        }
        break;
#endif // ASMJIT_X64
      ...
      case VARIABLE_TYPE_MM:
        ...
        {
          case VARIABLE_TYPE_GPD:
            compiler->_emit(INST_MOVD, gpd(dst), gpd(src));
            return;
#if defined(ASMJIT_X64)
          case VARIABLE_TYPE_GPQ:
            compiler->_emit(INST_MOVQ, gpq(dst), gpq(src));
            return;
#endif // ASMJIT_X64
          case VARIABLE_TYPE_MM:
            compiler->_emit(INST_MOVQ, mm(dst), mm(src));
            return;
        }
        break;
      ...
      case VARIABLE_TYPE_XMM_2D:
        ...
        {
          case VARIABLE_TYPE_GPD:
            compiler->_emit(INST_MOVD, xmm(dst), gpd(src));
            return;
#if defined(ASMJIT_X64)
          case VARIABLE_TYPE_GPQ:
            compiler->_emit(INST_MOVQ, xmm(dst), gpq(src));
            return;
#endif // ASMJIT_X64
          case VARIABLE_TYPE_MM:
            compiler->_emit(INST_MOVQ, xmm(dst), mm(src));
            return;
          case VARIABLE_TYPE_XMM:
          case VARIABLE_TYPE_XMM_1F:
          case VARIABLE_TYPE_XMM_4F:
          case VARIABLE_TYPE_XMM_1D:
          case VARIABLE_TYPE_XMM_2D:
            compiler->_emit(INST_MOVDQA, xmm(dst), xmm(src));
            return;
        }
        break;
      ...
      case VARIABLE_TYPE_XMM_1F:
        switch (vdata->type)
        {
          ...
          case VARIABLE_TYPE_XMM_1F:
          case VARIABLE_TYPE_XMM_4F:
            compiler->_emit(INST_MOVSS, xmm(dst), xmm(src));
            return;
          case VARIABLE_TYPE_XMM_1D:
          case VARIABLE_TYPE_XMM_2D:
            compiler->_emit(INST_CVTSD2SS, xmm(dst), xmm(src));
            return;
        }
        break;
      ...
      case VARIABLE_TYPE_XMM_1D:
        switch (vdata->type)
        {
          ...
          case VARIABLE_TYPE_XMM_1F:
          case VARIABLE_TYPE_XMM_4F:
            compiler->_emit(INST_CVTSS2SD, xmm(dst), xmm(src));
            return;
          case VARIABLE_TYPE_XMM_1D:
          case VARIABLE_TYPE_XMM_2D:
            compiler->_emit(INST_MOVSD, xmm(dst), xmm(src));
            return;
        }
        break;
    }
    ...
    switch (argType.variableType)
    {
      case VARIABLE_TYPE_GPD:
        switch (vdata->type)
        {
          ...
          case VARIABLE_TYPE_GPQ:
#endif // ASMJIT_X64
            compiler->_emit(INST_MOV, gpd(dst), mem);
            return;
          case VARIABLE_TYPE_MM:
            compiler->_emit(INST_MOVD, gpd(dst), mem);
            return;
        }
        break;
      ...
      case VARIABLE_TYPE_GPQ:
        switch (vdata->type)
        {
          case VARIABLE_TYPE_GPD:
            ...
            return;
          case VARIABLE_TYPE_GPQ:
            compiler->_emit(INST_MOV, gpq(dst), mem);
            return;
          case VARIABLE_TYPE_MM:
            compiler->_emit(INST_MOVQ, gpq(dst), mem);
            return;
        }
        break;
#endif // ASMJIT_X64
...
#endif // ASMJIT_X64
          case VARIABLE_TYPE_MM:
            compiler->_emit(INST_MOVQ, mm(dst), mem);
            return;
        }
        break;
      ...
      case VARIABLE_TYPE_XMM_2D:
        ...
        {
          case VARIABLE_TYPE_GPD:
            compiler->_emit(INST_MOVD, xmm(dst), mem);
            return;
#if defined(ASMJIT_X64)
          case VARIABLE_TYPE_GPQ:
            compiler->_emit(INST_MOVQ, xmm(dst), mem);
            return;
#endif // ASMJIT_X64
          case VARIABLE_TYPE_MM:
            compiler->_emit(INST_MOVQ, xmm(dst), mem);
          ...
          case VARIABLE_TYPE_XMM_1D:
          case VARIABLE_TYPE_XMM_2D:
            compiler->_emit(INST_MOVDQA, xmm(dst), mem);
            return;
        }
        break;
      ...
      case VARIABLE_TYPE_XMM_1F:
        switch (vdata->type)
        {
          ...
          case VARIABLE_TYPE_XMM_1F:
          case VARIABLE_TYPE_XMM_4F:
            compiler->_emit(INST_MOVSS, xmm(dst), mem);
            return;
          case VARIABLE_TYPE_XMM_1D:
          case VARIABLE_TYPE_XMM_2D:
            compiler->_emit(INST_CVTSD2SS, xmm(dst), mem);
            return;
        }
        break;
      ...
      case VARIABLE_TYPE_XMM_1D:
        switch (vdata->type)
        {
          ...
          case VARIABLE_TYPE_XMM_1F:
          case VARIABLE_TYPE_XMM_4F:
            compiler->_emit(INST_CVTSS2SD, xmm(dst), mem);
            return;
          case VARIABLE_TYPE_XMM_1D:
          case VARIABLE_TYPE_XMM_2D:
            compiler->_emit(INST_MOVSD, xmm(dst), mem);
            return;
        }
        break;
    }
        ...
        if (_ret[i].isVar())
        {
          if (reinterpret_cast<const BaseVar&>(_ret[i]).isGPVar())
          {
            ...
            if (srcIndex == INVALID_VALUE)
              compiler->_emit(INST_MOV, gpn(dstIndex), cc._getVarMem(vdata));
            else if (dstIndex != srcIndex)
              compiler->_emit(INST_MOV, gpn(dstIndex), gpn(srcIndex));
          }
        }
        else if (_ret[i].isImm())
        {
          compiler->_emit(INST_MOV, gpn(dstIndex), _ret[i]);
        }
      }
      break;
        ...
        if (_ret[i].isVar())
        {
          ...
          else if (reinterpret_cast<const BaseVar&>(_ret[i]).isXMMVar())
          {
            ...
            switch (vdata->type)
            {
              case VARIABLE_TYPE_XMM_1F:
              case VARIABLE_TYPE_XMM_4F:
                compiler->_emit(INST_FLD, _BaseVarMem(reinterpret_cast<BaseVar&>(_ret[i]), 4));
                break;
              case VARIABLE_TYPE_XMM_1D:
              case VARIABLE_TYPE_XMM_2D:
                compiler->_emit(INST_FLD, _BaseVarMem(reinterpret_cast<BaseVar&>(_ret[i]), 8));
                break;
            }
          }
        }
        ...
        if (_ret[i].isVar())
        {
          if (reinterpret_cast<const BaseVar&>(_ret[i]).isGPVar())
          {
            ...
            if (srci == INVALID_VALUE)
              compiler->_emit(inst, mm(dsti), cc._getVarMem(vdata));
            else
#if defined(ASMJIT_X86)
              compiler->_emit(inst, mm(dsti), gpd(srci));
#else
              compiler->_emit(inst, mm(dsti), _ret[i].isRegType(REG_TYPE_GPQ) ? gpq(srci) : gpd(srci));
#endif
          }
          else if (reinterpret_cast<const BaseVar&>(_ret[i]).isMMVar())
          {
            ...
            if (srci == INVALID_VALUE)
              ...
              compiler->_emit(inst, mm(dsti), mm(srci));
          }
          else if (reinterpret_cast<const BaseVar&>(_ret[i]).isXMMVar())
          {
            VarData* vdata = compiler->_getVarData(_ret[i].getId());
            ...
            if (srci == INVALID_VALUE)
              ...
              compiler->_emit(inst, mm(dsti), xmm(srci));
          }
        }
      }
      break;
      ...
      for (i = 0; i < 2; i++)
      {
        ...
        if (_ret[i].isVar())
        {
          if (reinterpret_cast<const BaseVar&>(_ret[i]).isGPVar())
          {
            ...
            if (srci == INVALID_VALUE)
              compiler->_emit(inst, xmm(dsti), cc._getVarMem(vdata));
            else
#if defined(ASMJIT_X86)
              compiler->_emit(inst, xmm(dsti), gpd(srci));
#else
              compiler->_emit(inst, xmm(dsti), _ret[i].isRegType(REG_TYPE_GPQ) ? gpq(srci) : gpd(srci));
#endif
          }
          ...
          else if (reinterpret_cast<const BaseVar&>(_ret[i]).isMMVar())
          {
            ...
            srci = vdata->registerIndex;
            if (srci == INVALID_VALUE)
              compiler->_emit(INST_MOVQ, xmm(dsti), cc._getVarMem(vdata));
            else
              compiler->_emit(INST_MOVQ, xmm(dsti), mm(srci));
          }
          else if (reinterpret_cast<const BaseVar&>(_ret[i]).isXMMVar())
          {
            VarData* vdata = compiler->_getVarData(_ret[i].getId());
            ...
            srci = vdata->registerIndex;
            if (srci == INVALID_VALUE)
              compiler->_emit(INST_MOVDQA, xmm(dsti), cc._getVarMem(vdata));
            else if (dsti != srci)
              compiler->_emit(INST_MOVDQA, xmm(dsti), xmm(srci));
          }
        }
      }
      break;
        ...
        if (_ret[i].isVar())
        {
          ...
          else if (reinterpret_cast<const BaseVar&>(_ret[i]).isXMMVar())
          {
            ...
            switch (vdata->type)
            {
              ...
              case VARIABLE_TYPE_XMM_4F:
                if (srci == INVALID_VALUE)
                  compiler->_emit(INST_MOVSS, xmm(dsti), cc._getVarMem(vdata));
                else
                  compiler->_emit(INST_MOVSS, xmm(dsti), xmm(srci));
                break;
              case VARIABLE_TYPE_XMM_1D:
              case VARIABLE_TYPE_XMM_2D:
                if (srci == INVALID_VALUE)
                  compiler->_emit(INST_CVTSD2SS, xmm(dsti), cc._getVarMem(vdata));
                else if (dsti != srci)
                  compiler->_emit(INST_CVTSD2SS, xmm(dsti), xmm(srci));
                break;
            }
          }
        }
        ...
        if (_ret[i].isVar())
        {
          ...
          else if (reinterpret_cast<const BaseVar&>(_ret[i]).isXMMVar())
          {
            ...
            switch (vdata->type)
            {
              ...
              case VARIABLE_TYPE_XMM_4F:
                if (srci == INVALID_VALUE)
                  compiler->_emit(INST_CVTSS2SD, xmm(dsti), cc._getVarMem(vdata));
                else
                  compiler->_emit(INST_CVTSS2SD, xmm(dsti), xmm(srci));
                break;
              case VARIABLE_TYPE_XMM_1D:
              case VARIABLE_TYPE_XMM_2D:
                if (srci == INVALID_VALUE)
                  compiler->_emit(INST_MOVSD, xmm(dsti), cc._getVarMem(vdata));
                else
                  compiler->_emit(INST_MOVSD, xmm(dsti), xmm(srci));
                break;
            }
          }
        }
...
void ERet::_emit(Assembler& a) ASMJIT_NOTHROW
{
  if (shouldEmitJumpToEpilog())
  {
...
void CompilerContext::emitLoadVar(VarData* vdata, uint32_t regIndex) ASMJIT_NOTHROW
{
  ...
  {
    case VARIABLE_TYPE_GPD:
      _compiler->_emit(INST_MOV, gpd(regIndex), m);
      if (_emitComments) goto addComment;
      break;
#if defined(ASMJIT_X64)
    case VARIABLE_TYPE_GPQ:
      _compiler->_emit(INST_MOV, gpq(regIndex), m);
      if (_emitComments) goto addComment;
      break;
#endif // ASMJIT_X64
    ...
    case VARIABLE_TYPE_MM:
      _compiler->_emit(INST_MOVQ, mm(regIndex), m);
      if (_emitComments) goto addComment;
      break;

    case VARIABLE_TYPE_XMM:
      _compiler->_emit(INST_MOVDQA, xmm(regIndex), m);
      if (_emitComments) goto addComment;
      break;
    case VARIABLE_TYPE_XMM_1F:
      _compiler->_emit(INST_MOVSS, xmm(regIndex), m);
      if (_emitComments) goto addComment;
      break;
    case VARIABLE_TYPE_XMM_1D:
      _compiler->_emit(INST_MOVSD, xmm(regIndex), m);
      if (_emitComments) goto addComment;
      break;
    case VARIABLE_TYPE_XMM_4F:
      _compiler->_emit(INST_MOVAPS, xmm(regIndex), m);
      if (_emitComments) goto addComment;
      break;
    case VARIABLE_TYPE_XMM_2D:
      _compiler->_emit(INST_MOVAPD, xmm(regIndex), m);
      if (_emitComments) goto addComment;
      break;
  }
  return;
...
void CompilerContext::emitSaveVar(VarData* vdata, uint32_t regIndex) ASMJIT_NOTHROW
{
  ...
  {
    case VARIABLE_TYPE_GPD:
      _compiler->_emit(INST_MOV, m, gpd(regIndex));
      if (_emitComments) goto addComment;
      break;
#if defined(ASMJIT_X64)
    case VARIABLE_TYPE_GPQ:
      _compiler->_emit(INST_MOV, m, gpq(regIndex));
      if (_emitComments) goto addComment;
      break;
#endif // ASMJIT_X64
    ...
    case VARIABLE_TYPE_MM:
      _compiler->_emit(INST_MOVQ, m, mm(regIndex));
      if (_emitComments) goto addComment;
      break;

    case VARIABLE_TYPE_XMM:
      _compiler->_emit(INST_MOVDQA, m, xmm(regIndex));
      if (_emitComments) goto addComment;
      break;
    case VARIABLE_TYPE_XMM_1F:
      _compiler->_emit(INST_MOVSS, m, xmm(regIndex));
      if (_emitComments) goto addComment;
      break;
    case VARIABLE_TYPE_XMM_1D:
      _compiler->_emit(INST_MOVSD, m, xmm(regIndex));
      if (_emitComments) goto addComment;
      break;
    case VARIABLE_TYPE_XMM_4F:
      _compiler->_emit(INST_MOVAPS, m, xmm(regIndex));
      if (_emitComments) goto addComment;
      break;
    case VARIABLE_TYPE_XMM_2D:
      _compiler->_emit(INST_MOVAPD, m, xmm(regIndex));
      if (_emitComments) goto addComment;
      break;
  }
  return;
...
void CompilerContext::emitMoveVar(VarData* vdata, uint32_t regIndex, uint32_t vflags) ASMJIT_NOTHROW
{
  ...
  {
    case VARIABLE_TYPE_GPD:
      _compiler->_emit(INST_MOV, gpd(regIndex), gpd(vdata->registerIndex));
      break;
#if defined(ASMJIT_X64)
    case VARIABLE_TYPE_GPQ:
      _compiler->_emit(INST_MOV, gpq(regIndex), gpq(vdata->registerIndex));
      break;
#endif // ASMJIT_X64
    ...
    case VARIABLE_TYPE_MM:
      _compiler->_emit(INST_MOVQ, mm(regIndex), mm(vdata->registerIndex));
      break;

    case VARIABLE_TYPE_XMM:
      _compiler->_emit(INST_MOVDQA, xmm(regIndex), xmm(vdata->registerIndex));
      break;
    case VARIABLE_TYPE_XMM_1F:
      _compiler->_emit(INST_MOVSS, xmm(regIndex), xmm(vdata->registerIndex));
      break;
    case VARIABLE_TYPE_XMM_1D:
      _compiler->_emit(INST_MOVSD, xmm(regIndex), xmm(vdata->registerIndex));
      break;
    case VARIABLE_TYPE_XMM_4F:
      _compiler->_emit(INST_MOVAPS, xmm(regIndex), xmm(vdata->registerIndex));
      break;
    case VARIABLE_TYPE_XMM_2D:
      _compiler->_emit(INST_MOVAPD, xmm(regIndex), xmm(vdata->registerIndex));
      break;
  }
}
...
void CompilerContext::emitExchangeVar(VarData* vdata, uint32_t regIndex, uint32_t vflags, VarData* other) ASMJIT_NOTHROW
{
  ...
  {
    case VARIABLE_TYPE_GPD:
      _compiler->_emit(INST_XCHG, gpd(regIndex), gpd(vdata->registerIndex));
      break;
#if defined(ASMJIT_X64)
    case VARIABLE_TYPE_GPQ:
      _compiler->_emit(INST_XCHG, gpq(regIndex), gpq(vdata->registerIndex));
      break;
#endif // ASMJIT_X64
    ...
    case VARIABLE_TYPE_MM:
    {
      ...
      _compiler->_emit(INST_PXOR, a, b);
      _compiler->_emit(INST_PXOR, b, a);
      _compiler->_emit(INST_PXOR, a, b);
      break;
    }
    ...
    case VARIABLE_TYPE_XMM_4F:
    {
      ...
      _compiler->_emit(INST_XORPS, a, b);
      _compiler->_emit(INST_XORPS, b, a);
      _compiler->_emit(INST_XORPS, a, b);
      break;
    }
    ...
    case VARIABLE_TYPE_XMM_2D:
    {
      ...
      _compiler->_emit(INST_XORPD, a, b);
      _compiler->_emit(INST_XORPD, b, a);
      _compiler->_emit(INST_XORPD, a, b);
      break;
    }
